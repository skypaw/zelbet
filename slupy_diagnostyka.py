...
def abc_diagnostyka(e_s2_func, a2_func, lambda_bet_func, plus_minus_1, f_yd_func, a_s_1_2, e_s_1_2, epsilon_cu3_func,
...
a_s1 = float(input("Zbrojenie dolne [cm^4]: ")) * 10 ** -4
a_s2 = float(input("Zbrojenie górne [cm^4]: ")) * 10 ** -4
...
if e <= emin:
    a_s1_1 = a_s1   #BŁĄD TRZEBA POPRAWIĆ NADPISYWANIE!!
    a_s2_1 = a_s2
    a_s1 = a_s2_1
    a_s2 = a_s1_1
...
x = 1 / lambda_bet * (
            -(e_s2 - a2) + np.sqrt((e_s2 - a2) ** 2 + (2 * f_yd * (a_s1 * e_s1 - a_s2 * e_s2)) / (eta_bet * f_cd * b)))
print(f"x {x}")
...
if x <= x_lim:
    if x < x_min_yd:
        ...
        if x <= x_min_minus_yd:
            x = 1 / lambda_bet * (-(e_s2 - a2) + np.sqrt((e_s2 - a2) ** 2 + (2 * f_yd * (a_s1 * e_s1 + a_s2 * e_s2)) / (
    ...
    A, B, C = abc_diagnostyka(e_s2, a2, lambda_bet, 1, f_yd, a_s2, e_s2, epsilon_cu3, es, a_s1, e_s1, eta_bet, f_cd, b,
                              d)
    print(f"A {A}")
    print(f"B {B}")
    print(f"C {C}")
...
m_rd = (eta_bet * f_cd * b * lambda_bet * x * (d - 0.5 * lambda_bet * x) + sigma_s2 * a_s2 * (d - a2) - n_ed * 10 ** -3 * (
            0.5 * h - a1)) * 10 ** 3
